(ns commtrade.operation
  "OperationActor -- one commission-brokerage operation = one supervised
  actor run, expressed as a langgraph-clj StateGraph. The advisor
  (CommTradeAdvisor) is sealed into a single node (:advise); its
  proposal is ALWAYS routed through the Commission Broker Governor
  ...
  One graph run = one commission-brokerage operation (intake -> advise ->
  govern -> decide -> commit | hold | approval). No unbounded inner
  loop -- each operation is auditable and checkpointed. A brokerage-
  deal's life is advanced by MANY ops (intake / verify / confirm /
  invoice), each its own independent run.
  ...
  `:deal/confirm`/`:commission/invoice` ALWAYS reach this node when
  the governor is clean -- see `commtrade.phase`."
  (:require [langgraph.graph :as g]
            [langgraph.checkpoint :as cp]
            [commtrade.commtradeadvisor :as commtradeadvisor]
...
(defn build
  ...
  `commtrade.store/Store`).
  opts:
    :advisor      -- a `commtrade.commtradeadvisor/Advisor` (default: mock-advisor)
    :checkpointer -- langgraph checkpointer (default: in-mem)"
  [store & [{:keys [advisor checkpointer]
             :or   {advisor      (commtradeadvisor/mock-advisor)
  ...
  (-> (g/state-graph
      ...
      ;; CommTradeAdvisor inference (the contained intelligence node) -- proposal only.
      (g/add-node :advise
        (fn [{:keys [request]}]
          (let [p (commtradeadvisor/-advise advisor store request)]
            {:proposal p :audit [(commtradeadvisor/trace request p)]})))

      ;; Commission Broker Governor -- independent censor (separate system than the LLM).
      (g/add-node :govern
